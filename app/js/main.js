...
$(function() {
  ...
  // Scroll to top function in the blog posts
  $(".hp-blog-scrollup").click(function() {
    console.log($("#hp-blog-empty-top").offset().top);
    $('html, body').animate({
        scrollTop: $("#hp-blog-empty-top").offset().top
    }, 2000);
  });
  ...
  if ( $( "#hp-map-canyonlands" ).length ) {
    L.mapbox.accessToken = 'pk.eyJ1IjoiaG1wYXJhbmphcGUiLCJhIjoiY2lnOHJ3dGt2MHE5d3RobTA4bG1kNzcwOSJ9.a-n4ERL-LSgzsQv0RBbMfQ';
    $.getJSON($( "#hp-map-canyonlands" ).attr('data-geojson'), function(jsondata){
      var map_2 = L.mapbox.map('hp-map-canyonlands', 'mapbox.streets')
        .setView([38.5, -109.5], 8);
        var myLayer = L.mapbox.featureLayer().addTo(map_2);
  ...
  if ( $( "#hp-map-zion-bryce" ).length ) {
    L.mapbox.accessToken = 'pk.eyJ1IjoiaG1wYXJhbmphcGUiLCJhIjoiY2lnOHJ3dGt2MHE5d3RobTA4bG1kNzcwOSJ9.a-n4ERL-LSgzsQv0RBbMfQ';
    $.getJSON($( "#hp-map-zion-bryce" ).attr('data-geojson'), function(jsondata){
      var map_2 = L.mapbox.map('hp-map-zion-bryce', 'mapbox.streets')
        .setView([36.9, -111.5], 8);
        var myLayer = L.mapbox.featureLayer().addTo(map_2);
